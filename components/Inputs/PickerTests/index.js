// import { DISTANCES, STYLES } from '@/legasy/src/CONSTANTS/SWIMMING_TESTS'
// import { DoneIcon } from '@/legasy/src/utils/Icons'
        ...
        <div key={styleId} className="flex flex-col w-1/6 ">
          ...
          {DISTANCES.map(({ id, label: distance }) => (
            <Cell size={compact ? 'sm' : 'lg'} style="small" key={id}>
              ...
              >
                <div
                  className={`w-full h-full flex justify-center items-center ${isCurrentlySelected({ distance, style: styleId }) &&
                    'border-2'
                    }`}
                  onClick={(e) => {
                    e.preventDefault()
